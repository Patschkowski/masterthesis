#ifndef DOUBLE_BUFFERING_TEMPLATE_HPP
#define DOUBLE_BUFFERING_TEMPLATE_HPP

#include "KernelTemplate.hpp"

template< typename Matrix >
class DoubleBufferingTemplate : public KernelTemplate< Matrix >
{
public :
  void generate( std::ostream& os, Matrix const& lhs, Matrix const& rhs, Matrix const& result) const override
  {
    ...
    oss << "  local "   << dataType << " Alm0[" << this->Mwg() * this->Kwg() / 2 << "], Blm0[" << this->Kwg() * this->Nwg() / 2 << "];\n" << 
           "  local "   << dataType << " Alm1[" << this->Mwg() * this->Kwg() / 2 << "], Blm1[" << this->Kwg() * this->Nwg() / 2 << "];\n" <<
    ...
    loadIntoLocal0( oss, lhs, rhs );
      
    oss << "\n  for ( ; pwg <= " << lhs.cols() - 2u * this->Kwg() << "; pwg += " << this->Kwg() << ")\n  {\n";
      
    oss << "    barrier( CLK_LOCAL_MEM_FENCE );\n\n";
    
    // Load subblocks of A and B collectively.
    loadIntoLocal1( oss, lhs, rhs );
      
    oss << "\n    for ( size_t pwi = 0; pwi <= " << this->Kwg()/2 - this->Kwi() << "; pwi += " << this->Kwi() << " )\n    {\n";
    ...
    oss << "    for ( size_t pwi = " << this->Kwg() / 2 << "; pwi <= " << this->Kwg() - this->Kwi() << "; pwi += " << this->Kwi() << " )\n    {\n";
  ...
  DoubleBufferingTemplate( size_t stride ): 
    DoubleBufferingTemplate( 
                   96, 96, 16,
                   2,
                   16, 16,
                   16, 16,
...
private :
  ...
  void loadIntoLocal1( std::ostream& oss, Matrix const& lhs, Matrix const& rhs ) const
  {
    for ( size_t i = 0; i < this->MwiA(); ++i )
      for ( size_t j = this->KwiA() / 2; j < this->KwiA(); ++j )
      {
        auto const indexX = std::to_string( this->KwiA() ) + " * lx + " + std::to_string( j );
        auto const indexY = std::to_string( this->MwiA() ) + " * ly + " + std::to_string( i );
        
        oss << "    Alm1[" << 
          index( indexX + " - " + std::to_string( this->KwiA() ), indexY, this->Kwg(), this->Mwg() ) << "] = A[" << 
          index( indexX + " + pwg",
                indexY + " + gy * " + std::to_string( this->Mwg() ), lhs ) << "];\n";
      }
                
    for ( size_t i = this->KwiB() / 2; i < this->KwiB(); ++i )
      for ( size_t j = 0; j < this->NwiB(); ++j )
      {
        ...
        oss << "    Blm1[" <<
          index( indexX, indexY + " - " + std::to_string( this->KwiB() ), this->Nwg(), this->Kwg() ) << "] = B[" << 
          index( indexX + " + gx * " + std::to_string( this->Nwg() ),
                indexY + " + pwg", rhs ) << "];\n";
      }
  }
  
  void loadIntoLocal0( std::ostream& oss, Matrix const& lhs, Matrix const& rhs ) const
  {
    // Load subblocks of A and B collectively.
    for ( size_t i = 0; i < this->MwiA(); ++i )
      for ( size_t j = 0; j < this->KwiA() / 2; ++j )
      {
        auto const indexX = std::to_string( this->KwiA() ) + " * lx + " + std::to_string( j );
        auto const indexY = std::to_string( this->MwiA() ) + " * ly + " + std::to_string( i );
    ...
    for ( size_t i = 0; i < this->KwiB() / 2; ++i )
      for ( size_t j = 0; j < this->NwiB(); ++j )
      {
        auto const indexX = std::to_string( this->NwiB() ) + " * lx + " + std::to_string( j );
        auto const indexY = std::to_string( this->KwiB() ) + " * ly + " + std::to_string( i );
  ...
  void loadFromLocal0( std::ostream& oss ) const
  {
    for ( size_t i = 0; i < this->Mwi(); ++i )
      for ( size_t j = 0; j < this->Kwi(); ++j )
        oss << "      Apm[" << index( std::to_string( j ), std::to_string( i ), this->Kwi(), this->Mwi() ) << "] = Alm0[" << 
          index( 
            "pwi + " + std::to_string( j ), 
    ...
    for ( size_t i = 0; i < this->Kwi(); ++i )
      for ( size_t j = 0; j < this->Nwi(); ++j )
        oss << "      Bpm[" << index( std::to_string( j ), std::to_string( i ), this->Nwi(), this->Kwi() ) << "] = Blm0[" << 
          index( 
            std::to_string( this->Nwi() ) + " * lx + " + std::to_string( j ),
            "pwi + " + std::to_string( i ), this->Nwg(), this->Kwg() ) << "];\n";
  }
  
  void loadFromLocal1( std::ostream& oss ) const
  {
    for ( size_t i = 0; i < this->Mwi(); ++i )
      for ( size_t j = 0; j < this->Kwi(); ++j )
        oss << "      Apm[" << index( std::to_string( j ), std::to_string( i ), this->Kwi(), this->Mwi() ) << "] = Alm1[" << 
          index( 
            "pwi + " + std::to_string( j ), 
            std::to_string( this->Mwi() ) + " * ly + " + std::to_string( i ), this->Kwg(), this->Mwg() ) << "];\n";
    ...
    for ( size_t i = 0; i < this->Kwi(); ++i )
      for ( size_t j = 0; j < this->Nwi(); ++j )
        oss << "      Bpm[" << index( std::to_string( j ), std::to_string( i ), this->Nwi(), this->Kwi() ) << "] = Blm1[" << 
          index( 
            std::to_string( this->Nwi() ) + " * lx + " + std::to_string( j ),
            "pwi + " + std::to_string( i ), this->Nwg(), this->Kwg() ) << "];\n";
  }
