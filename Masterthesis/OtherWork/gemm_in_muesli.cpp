...
struct Result
{
  ...
  friend std::ostream& 
  operator <<( std::ostream& o, Result const& r )
  {
 ...
 */
inline static Type 
extractRow( Matrix const& mat, size_t rowIndex, int colIndex )
{
 ...
 */
inline static Type
extractCol( Matrix const& mat, size_t colIndex, int rowIndex )
{
  return mat.at( rowIndex, colIndex );
}
 ...
 */
static Result* 
dotProduct( Matrix const& lhs, Matrix const& rhs, Indices* idx )
{  
  Vector row( lhs.cols(),
              msl::curry( extractRow )( lhs )( idx->row ) );
  Vector col( rhs.rows(), 
              msl::curry( extractCol )( rhs )( idx->col ) );
  
  Vector dot = row.zipWith( col, mul< Type > );
  
  return new Result{ dot.fold( msl::add< Type > ),
                     idx->row, idx->col };
}
...
{
  // Cast away constness as Muesli does
  // not support non-const references as arguments ...
...
int main( int argc, char** argv )
{
  ...
  { 
    ...
    msl::Initial< Indices > source(
      msl::curry( generateIndices )( C.rows() )( C.cols() ) );
    
    msl::Atomic< Indices, Result > executeDotProduct( 
      msl::curry( dotProduct )( A )( B ), 1 );
    
    msl::Final< Result > sink( 
      msl::curry( collectResults )( C ) );
